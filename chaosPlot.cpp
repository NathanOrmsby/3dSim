...
void gramSchmidtRenormalize(Probe *probes, double *areaSums, int probeNum, double initialArea, int NUMPOINTS)
{
	for (int i = 0; i < probeNum; ++i)
	{
		...
		probes[i].perturbed[NUMPOINTS].x = u1[0]; probes[i].perturbed[NUMPOINTS].z = u1[1]; probes[i].perturbed[NUMPOINTS + 1].x = y2[0] / magy2; probes[i].perturbed[NUMPOINTS + 1].z = y2[1] / magy2;
	}
}
